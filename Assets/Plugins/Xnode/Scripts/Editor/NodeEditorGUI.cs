...
namespace XNodeEditor {
    ...
    public partial class NodeEditorWindow {
        ...
        protected virtual void OnGUI() {
            ...
            NodeSettingInfo.FunctionColor = NodeEditorPreferences.GetSettings().FunctionColor;
            NodeSettingInfo.BranchColor = NodeEditorPreferences.GetSettings().BranchColor;
            NodeSettingInfo.ValueColor = NodeEditorPreferences.GetSettings().ValueColor;
            style.normal.textColor = Color.white;
            style.fontSize = 40; 
            GUI.Label(new Rect(10, 10, 200, 200), graph.Name, style);
            style.fontSize = 15;
            GUI.Label(new Rect(10, 60, 200, 800), "存放位置: "+graph.Path  +" (这是实验性功能，可能有崩溃危险)", style);
            if (Application.isPlaying)
            {
                style.normal.textColor = new Color(0.97f, 0.65f, 0.17f, 1);
                style.fontSize = 15;
                GUI.Label(new Rect(10, 90, 200, 800), "游戏已经在运行，但你仍可以在此处修改你的程序表并直接看到修改结果", style);
            }
        ...
        void ShowRerouteContextMenu(RerouteReference reroute) {
            ...
            contextMenu.AddItem(new GUIContent("删除节点"), false, () => reroute.RemovePoint());
        ...
        void ShowPortContextMenu(XNode.NodePort hoveredPort) {
            ...
            foreach (var port in hoveredPort.GetConnections()) {
                ...
                contextMenu.AddItem(new GUIContent($"断开与({name})的联结"), false, () => hoveredPort.Disconnect(index));
            }
            contextMenu.AddItem(new GUIContent("断开此节点的所有联结"), false, () => hoveredPort.ClearConnections());
            contextMenu.DropDown(new Rect(Event.current.mousePosition, Vector2.zero));
            if (NodeEditorPreferences.GetSettings().autoSave) AssetDatabase.SaveAssets();
        }
        ...
        private void DrawNodes() {
            ...
            for (int n = 0; n < graph.nodes.Count; n++) {
                ...
                if (NodeName.Contains("事件") || NodeName.Contains("函数"))
                    node.TintColor = NodeSettingInfo.FunctionColor;
                if (NodeName.Contains("获取") || NodeName.Contains("设置") || NodeName.Contains("数组") || NodeName.Contains("获得"))
                    node.TintColor = NodeSettingInfo.ValueColor;
                if (NodeName.Contains("判断") || NodeName.Contains("比较") || NodeName.Contains("限制执行次数") || NodeName.Contains("循环") || NodeName.Contains("分支"))
                    node.TintColor = NodeSettingInfo.BranchColor;
                //Get node position
                ...
                if (selected) {
                    ...
                    GUI.color = (node.TintColor == Color.white ? nodeEditor.GetTint() : node.TintColor);
