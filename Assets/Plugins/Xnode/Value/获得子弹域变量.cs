	...
	public class 获得子弹域变量 : Node
	{
		...
		public void 刷新()
		{
			...
			switch (类型)
			{
				case ValueType.三维向量:
					if (不声明数组)
						AddDynamicOutput(typeof(Vector3), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<Vector3>), ConnectionType.Multiple, TypeConstraint.None, "输出");
					break;
				case ValueType.整数:
					if (不声明数组)
						AddDynamicOutput(typeof(int), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<int>), ConnectionType.Multiple, TypeConstraint.None, "输出");
					break;
				case ValueType.文本:
					if (不声明数组)
						AddDynamicOutput(typeof(string), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<string>), ConnectionType.Multiple, TypeConstraint.None, "输出");
					break;
				case ValueType.激光:
					if (不声明数组)
						AddDynamicOutput(typeof(LazerShooting), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<LazerShooting>), ConnectionType.Multiple, TypeConstraint.None, "输出");

					break;
				case ValueType.发射器:
					if (不声明数组)
						AddDynamicOutput(typeof(Shooting), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<Shooting>), ConnectionType.Multiple, TypeConstraint.None, "输出");
					break;
				case ValueType.子弹:
					if (不声明数组)
						AddDynamicOutput(typeof(Bullet), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<Bullet>), ConnectionType.Multiple, TypeConstraint.None, "输出");
					break;
				case ValueType.布尔:
					if (不声明数组)
						AddDynamicOutput(typeof(bool), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<bool>), ConnectionType.Multiple, TypeConstraint.None, "输出");
					break;
				case ValueType.触发器:
					if (不声明数组)
						AddDynamicOutput(typeof(Trigger), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<Trigger>), ConnectionType.Multiple, TypeConstraint.None, "输出");
					break;
				case ValueType.力场:
					if (不声明数组)
						AddDynamicOutput(typeof(Force), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<Force>), ConnectionType.Multiple, TypeConstraint.None, "输出");
					break;
				case ValueType.浮点数:
					if (不声明数组)
						AddDynamicOutput(typeof(float), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<float>), ConnectionType.Multiple, TypeConstraint.None, "输出");
					break;
				case ValueType.敌人:
					if (不声明数组)
						AddDynamicOutput(typeof(Enemy), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<Enemy>), ConnectionType.Multiple, TypeConstraint.None, "输出");
					break;
				case ValueType.曲线:
					if (不声明数组)
						AddDynamicOutput(typeof(AnimationCurve), ConnectionType.Multiple, TypeConstraint.None, "输出");
					else
						AddDynamicOutput(typeof(List<AnimationCurve>), ConnectionType.Multiple, TypeConstraint.None, "输出");
					break;
			}
		}
		...
		{
			if (port.fieldName == "退出节点")
				return null;
			if (!Application.isPlaying)
				return null;
			子弹 = GetInputValue("子弹", 子弹);
			
			STGComponent component = (STGComponent)子弹;
